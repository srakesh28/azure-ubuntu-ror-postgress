...
# need to pass in the intended root password, the DNC credentials, and the instance ID
if [ $# -lt 8 ]; then
    echo $0: usage: xtoph_db_setup_script.sh [MysqlRootPassword] [DnccDb] [DnccUser] [DnccUserPass] [InstanceID] [MaintPassword] [masterIp] [otherIPs] [shareUrl] [shareUser] [sharePass] [sshusername]
    echo "Count was: " $#
	exit 1
fi
rootpass=$1
...
if [ $instanceId = "1" ]; then 
    ...
    #mount of the uploads share to allow for secondary web servers to sync

    echo "mounting upload share"
    uploadsMods=0555
    uploadMountLocation="/var/www/uploadshare"

    mkdir /var/www
    mkdir /var/www/uploadshare
    ...
    # create the dnccUser database and account
    mysql -u root -p"$rootpass" -e "create database $dnccDb;create user '$dnccUser'@'%' identified by '$dnccUserPass';grant all on $dnccDb.* to '$dnccUser'@'%';"
