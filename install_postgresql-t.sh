...
configure_streaming_replication() {
	...
	then
		...
		echo "archive_command = 'cp -i %p /var/lib/postgresql/9.4/main/archive/%f'" >> postgresql.conf
	...
	mkdir -p /var/lib/9.3/main/archive/
	...
	then
		...
		# Update postgressql.conf
		logger "Update postgressql.conf file"
		cd /var/lib/postgresql/9.3/main
		echo "listen_addresses = 'localhost, $SLAVEIP'" >> postgresql.conf
		echo "wal_level = hot_standby" >> postgresql.conf
		echo "checkpoint_segments = 8" >> postgresql.conf
		echo "max_wal_senders = 3" >> postgresql.conf
		echo "wal_keep_segments = 8" >> postgresql.conf
