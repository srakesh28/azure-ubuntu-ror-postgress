...
if [ $# -ne 7 ]; then
    echo $0: usage: setup_hyperdb_secondary.sh [DnccDb] [DnccUser] [DnccUserPass]  [masterIp] [LocalSlaves] [RemoteSlaves] [port]
...
dnccUserPass=$3
master=$4
localslaves=$( echo ${5} | cut -d '[' -f 2 | cut -d ']' -f 1 );
remoteslaves=$( echo ${6} | cut -d '[' -f 2 | cut -d ']' -f 1 );
port=$7


echo "database: " $dnccDb
echo "user: " $dnccUser
echo "dnccUserPass" $dnccUserPass
echo "master: " $master
echo "localslaves: " $localslaves
echo "remoteslaves: " $remoteslaves

echo "writing db-config"

conffile="/var/www/html/db-config.php"
...
echo "/* Local Slaves */" >> $conffile 
IFS=',' read -r -a servers <<< "$localslaves"
len=${#servers[@]}
for((n=0;n<$len;n++))
{
cat >> $conffile << EOF
...
echo "/* Remote Slaves */" >> $conffile 
IFS=',' read -r -a servers <<< "$remoteslaves"
len=${#servers[@]}
for((n=0;n<$len;n++))
{
cat >> $conffile << EOF
...
echo " # The ending PHP tag is omitted. This is actually safer than including it. " >> $conffile

echo "writing db.php"
cat > "/var/www/html/wp-content/db.php" << "EndText"
<?php
